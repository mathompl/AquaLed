...
static bool recvRetCommandFinished()
{
  bool ret = false;
  uint8_t temp[4] = {0};
...
int recvRetNumber()
{
  ...
  {
    r = (temp[4] << 24) | (temp[3] << 16) | (temp[2] << 8) | (temp[1]);
    recvRes = true;
    return  (int)r;
...
static int getNxInt (int &i)
{
  char b[11]  = {0};
  if (!recvRetString (b, 10, 1000)) return false;
  strcat (b, "\0");
  i = atoi(b);
  wdt_reset();
...
static int getNxStr (char *i)
{
  memset (i, 0,  sizeof (i));
  if (!recvRetString (i, 10, 1000)) return false;
  wdt_reset();
...
static void updateInfo() {
  ...
  fanAnimation++;
  if (fanAnimation > 3) fanAnimation = 0;
  byte aniFrame = 8 + fanAnimation;
  //  if (nxledFansStatus != ledFansStatus || forceRefresh || nxsumpFansStatus  != sumpFansStatus)
  {
    char temp[1] = {0};
    memset (temp, 0, sizeof (temp));
    if (waterFansStatus)
    {
      strcpy_P(temp , (PGM_P)pgm_read_word(&(nxConstStrings[aniFrame])));
    }
    else
    {
      strcpy_P(temp , (PGM_P)pgm_read_word(&(nxConstStrings[STR_EMPTY])));
    }
    sendCommandPGM (CMD_SET_T4, temp, NULL);
    ...
    sendCommandPGM (CMD_SET_T6, temp, NULL);
    ...
    sendCommandPGM (CMD_SET_T7, temp, NULL);
