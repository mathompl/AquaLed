...
void fansControl()
{
  ...
  if (temperatureFans != TEMP_ERROR) {
    if (currentTimeSec - previousMillisFans > LED_FANS_INTERVAL) {
  ...
  if (temperatureAqua != TEMP_ERROR) {
    if (currentTimeSec - previousMillisAqua > WATER_FANS_INTERVAL) {
  ...
  if (temperatureSump != TEMP_ERROR) {
    ...
    if (currentTimeSec - previousMillisSump > SUMP_FANS_INTERVAL) {
